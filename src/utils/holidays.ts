...
/**
 * Returns all public holidays for a given year and German Bundesland code (e.g. 'DE-BE')
 */
export function getGermanHolidays(year: number, stateCode: string = 'DE-BE'): HolidayInfo[] {
  ...
  const holidays: HolidayInfo[] = [
    // Bundesweite Feiertage (Standard)
    ...
    // 7. Allerheiligen (01.11.): BW, BY, NW, RP, SL
    { date: `${year}-11-01`, name: 'Allerheiligen', isNationwide: false, applicableStates: ['DE-BW', 'DE-BY', 'DE-NW', 'DE-RP', 'DE-SL'] },
  ];

  // 8. Buß- und Bettag (Mittwoch vor dem 23. November): SN
  // Let's calculate: 23. Nov minus (day of week of 23. Nov + 4) % 7
  const nov23 = new Date(Date.UTC(year, 10, 23));
  const dayOfWeek = nov23.getUTCDay(); // 0=Sun, 1=Mon, 2=Tue, 3=Wed, 4=Thu, 5=Fri, 6=Sat
...
/**
 * Checks if a specific date is a public holiday in the given state
 */
export function isGermanHoliday(dateStr: string, stateCode: string = 'DE-BE'): { isHoliday: boolean; holidayName?: string } {
...
/**
 * Counts working days (excluding weekends and state holidays) in a date range
 */
export function getWorkingDaysInRange(
  ...
  while (curr <= end) {
    ...
    const dayOfWeek = curr.getUTCDay() === 0 ? 7 : curr.getUTCDay(); // 1=Mon..7=Sun
