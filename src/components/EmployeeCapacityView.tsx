...
export const EmployeeCapacityView: React.FC = () => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-150">
      ...
      {/* Visual Capacity & Overbooking Monitor Card (Mobile Stacked & Expandable) */}
      {capacityData && (
        <div className="bg-white rounded-2xl border border-slate-200/90 p-4 sm:p-5 shadow-xs space-y-3">
          <div className="flex items-center justify-between border-b border-slate-100 pb-2.5">
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold text-xs text-slate-900">
                Team-Kapazitätsauslastung & Überbuchungs-Monitor ({capacityData.periodLabel})
              </span>
            </div>
            <button
              id="btn-expand-capacity-chart"
              onClick={() => setShowExpandedModal(true)}
              className="text-xs font-semibold text-blue-700 hover:text-blue-800 bg-blue-50 hover:bg-blue-100 px-2.5 py-1 rounded-lg flex items-center gap-1 transition-colors"
          ...
          <div className="space-y-2.5 pt-1">
            <div className="flex justify-between text-xs font-semibold">
              <span className="text-slate-600">Gesamtauslastung aller Mitarbeiter</span>
              <span className={`font-bold ${capacityData.kpis.overallUtilizationPercent > 100 ? 'text-rose-600' : 'text-blue-700'}`}>
                {capacityData.kpis.totalPlannedHours}h von {capacityData.kpis.totalCapacityHours}h ({capacityData.kpis.overallUtilizationPercent}%)
              </span>
            </div>
            ...
            <div className="flex flex-wrap items-center justify-between text-[11px] text-slate-500 pt-1">
              <span>{capacityData.kpis.totalEmployeesCount} Mitarbeiter erfasst</span>
              <span className={capacityData.kpis.totalOverbookedCount > 0 ? 'text-rose-600 font-bold' : 'text-emerald-700 font-semibold'}>
                {capacityData.kpis.totalOverbookedCount > 0
                  ? `${capacityData.kpis.totalOverbookedCount} Mitarbeiter überbucht (>100%)`
                  : 'Keine Überbuchungskonflikte'}
              </span>
            </div>
          </div>
        </div>
      ...
      {/* Full-Screen / Expanded Capacity Modal for Mobile & Tablets */}
      {showExpandedModal && capacityData && (
        <ChartExpandedModal
          title={`Kapazitäts- & Überbuchungsanalyse (${capacityData.periodLabel})`}
          subtitle={`Mandant: ${organization?.name || 'Insight Arcs'} • ${filteredEmployees.length} Mitarbeiter`}
          kpiList={[
            {
              label: 'Gesamtauslastung',
              value: `${capacityData.kpis.overallUtilizationPercent}%`,
              subtext: `${capacityData.kpis.totalPlannedHours}h von ${capacityData.kpis.totalCapacityHours}h`,
              color: capacityData.kpis.overallUtilizationPercent > 100 ? 'text-rose-600' : 'text-blue-700',
              icon: TrendingUp
            },
            {
              label: 'Überbuchte MA',
              value: capacityData.kpis.totalOverbookedCount,
              subtext: capacityData.kpis.totalOverbookedCount > 0 ? 'Konflikt >100%' : 'Alle im Soll',
              color: capacityData.kpis.totalOverbookedCount > 0 ? 'text-rose-600' : 'text-emerald-700',
              icon: ShieldAlert
            },
            {
              label: 'Soll-Kapazität',
              value: `${capacityData.kpis.totalCapacityHours}h`,
              subtext: 'Arbeitstage laut Feiertagskalender',
              color: 'text-slate-800',
              icon: Clock
            },
            {
              label: 'Geplante Stunden',
              value: `${capacityData.kpis.totalPlannedHours}h`,
              subtext: `Ist erfasst: ${capacityData.kpis.totalActualHours}h`,
            ...
            <div className="space-y-4 py-2">
              <div className="space-y-2">
                <div className="flex justify-between text-sm font-bold text-slate-800">
                  <span>Mitarbeiter-Auslastungsquote</span>
                  <span className="text-blue-700 font-extrabold">{capacityData.kpis.overallUtilizationPercent}%</span>
                </div>
              ...
              {/* Overbooked Member Warning Bar if any */}
              {capacityData.kpis.totalOverbookedCount > 0 && (
                <div className="bg-rose-50 border border-rose-200 rounded-xl p-3 text-rose-950 text-xs flex items-center justify-between">
                  <span className="font-bold flex items-center gap-1.5">
                    <ShieldAlert className="w-4 h-4 text-rose-600" />
                    Überbuchungs-Warnung: {capacityData.kpis.totalOverbookedCount} Mitarbeiter überschreiten ihre Kapazität.
                  </span>
                  <span className="text-rose-700 font-semibold bg-white px-2 py-0.5 rounded border border-rose-200">
                    Handlungsbedarf
                  </span>
                </div>
              )}
              ...
              <table className="w-full text-left text-xs">
                <thead className="bg-slate-50 text-slate-600 border-b border-slate-200 font-bold">
                  <tr>
                    ...
                    <th className="p-3 text-right">Soll-Std</th>
                ...
                <tbody className="divide-y divide-slate-100">
                  {filteredEmployees.map(emp => (
                    <tr key={emp.userId} className="hover:bg-slate-50">
                      <td className="p-3 font-semibold text-slate-800">{emp.userName}</td>
                      <td className="p-3 text-slate-500">{emp.role}</td>
                      <td className="p-3 text-right text-slate-600">{emp.targetCapacityHours}h</td>
                      <td className="p-3 text-right font-bold text-slate-900">{emp.totalPlannedHours}h</td>
                      <td className={`p-3 text-right font-bold ${emp.isOverbooked ? 'text-rose-600' : 'text-slate-800'}`}>
                      ...
                      <td className="p-3 text-center">
                        <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${
                          ...
                          {emp.isOverbooked ? `+${emp.overbookedHours.toFixed(1)}h Überbucht` : 'OK'}
                        </span>
                      </td>
                    </tr>
                  ))}
