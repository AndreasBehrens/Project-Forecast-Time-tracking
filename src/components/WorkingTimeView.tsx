    ...
    <div className="space-y-6">
            ...
            <div className="flex flex-wrap gap-2 pt-1">
              {summary.sanityDiscrepancies.map((d: any) => (
                <span key={d.date} className="bg-white/80 border border-amber-200 px-2 py-1 rounded-md font-medium text-[11px]">
                  📅 {d.date}: Projektzeit <strong>{d.projectHours}h</strong> &gt; Arbeitszeit <strong>{d.workHours}h</strong> (+{d.diff}h)
                </span>
              ))}
            </div>
            <p className="text-[11px] text-amber-700/80 pt-1">
              (Hinweis: Dies ist eine sanfte Warnung zur Vermeidung von Tippfehlern und blockiert keine Abrechnung oder Freigabe).
            </p>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white p-4 rounded-2xl border border-slate-200/90 shadow-xs space-y-1">
          ...
          <div className="text-[11px] text-slate-500">
            {summary?.targetWorkDays || 20} Arbeitstage ({selectedUserObj?.dailyTargetHours || 8}h/Tag)
          </div>
        </div>

        <div className="bg-white p-4 rounded-2xl border border-slate-200/90 shadow-xs space-y-1">
          <div className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">{t.actualHoursMonth} (Anwesenheit)</div>
          <div className="text-2xl font-bold text-slate-900">
            {summary?.actualWorkingHours || 0}h
          </div>
          <div className="text-[11px] text-emerald-600 font-medium">
            Erfasste Netto-Arbeitszeit
          </div>
        </div>

        <div className="bg-white p-4 rounded-2xl border border-slate-200/90 shadow-xs space-y-1">
          <div className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Erfasste Projektzeit</div>
          <div className="text-2xl font-bold text-slate-900">
            {summary?.actualProjectHours || 0}h
          </div>
          <div className="text-[11px] text-slate-500">
            Auf Kunden & Projekte gebucht
          </div>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200/90 shadow-sm p-5 space-y-4">
        <h3 className="text-sm font-bold text-slate-900 flex items-center gap-2">
          ...
          Tagesarbeitszeit erfassen / korrigieren
        </h3>

        <form onSubmit={handleSaveWorkingDay} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-3 items-end">
          <div className="lg:col-span-3">
            <label className="block text-[11px] font-semibold text-slate-500 mb-1">Datum</label>
            <input
              id="input-work-date"
              type="date"
              value={entryDate}
          ...
          <div className="lg:col-span-2">
            <label className="block text-[11px] font-semibold text-slate-500 mb-1">{t.clockIn} (Beginn)</label>
            <input
              id="input-work-start"
              type="time"
              value={startTime}
          ...
          <div className="lg:col-span-2">
            <label className="block text-[11px] font-semibold text-slate-500 mb-1">{t.clockOut} (Ende)</label>
            <input
              id="input-work-end"
              type="time"
              value={endTime}
      ...
      <div className="bg-white rounded-2xl border border-slate-200/90 shadow-xs overflow-hidden">
        <div className="px-5 py-3 border-b border-slate-200/80 font-bold text-xs text-slate-700 bg-slate-50/50">
          Protokollierte Tagesarbeitszeiten ({filteredEntries.length} Tage)
        </div>
          ...
          <table className="w-full text-left text-xs">
            <thead className="bg-slate-50 text-slate-500 border-b border-slate-200 font-semibold">
              <tr>
                ...
                <th className="px-4 py-3">Kommen - Gehen</th>
                <th className="px-4 py-3">Pause</th>
                <th className="px-4 py-3">Bruttozeit</th>
                <th className="px-4 py-3">Netto-Arbeitszeit</th>
                <th className="px-4 py-3">Arbeitsort / Notiz</th>
                ...
                <tr key={entry.id} className="hover:bg-slate-50/70 transition-colors">
                  ...
                  <td className="px-4 py-3 text-right">
                    <span className="inline-flex items-center gap-1 text-[10px] font-semibold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded-full border border-emerald-200/60">
                      <CheckCircle className="w-3 h-3" /> Konform
                    </span>
                  </td>
                </tr>
              ))}
