...
export const ForecastView: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'aggregated' | 'detail'>('aggregated');
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-slate-200/90 p-2 shadow-xs flex flex-col sm:flex-row items-center justify-between gap-3">
        ...
        >
          ...
          <span>
            Standort: <strong className="text-slate-800">{organization?.locationCity || 'Berlin'}</strong> ({organization?.stateLocation || 'DE-BE'})
          </span>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-2xl border border-slate-200/90 shadow-sm p-5 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1">
                {t.forecastSubtitle} (14-Tage Schwellenwert-Prüfung: 20%)
              </p>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              >
                ...
                Forecast anlegen / anpassen
              </button>
            </div>
          </div>

          {/* Threshold Exceeded Warning Banner (Section 21) */}
          {exceededCount > 0 && (
            <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 text-amber-950 flex items-start gap-3 shadow-xs">
              ...
              <div className="text-xs space-y-1">
                <div className="font-bold text-sm text-amber-900">
                  {t.thresholdWarning} ({exceededCount} Projekt-Ressourcen weichen &gt;20% ab)
                </div>
                <p className="text-amber-800 leading-relaxed">
                  Gemäß Vorgabe werden Projektleiter und Administratoren bei erstmaliger oder verschärfter Überschreitung benachrichtigt.
                  Die Hochrechnung basiert präzise auf den verstrichenen Arbeitstagen des Monats unter Berücksichtigung der Berliner Feiertage.
                </p>
              </div>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="bg-white p-4 rounded-2xl border border-slate-200/90 shadow-xs space-y-1">
              <div className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">{t.plannedHours} (Gesamt)</div>
              <div className="text-2xl font-bold text-slate-900">{totalPlannedHours.toFixed(1)}h</div>
              <div className="text-[11px] text-slate-500">Planung für {selectedMonth}</div>
            </div>

            <div className="bg-white p-4 rounded-2xl border border-slate-200/90 shadow-xs space-y-1">
              <div className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">{t.actualHours} (Bisher erfasst)</div>
              <div className="text-2xl font-bold text-slate-900">{totalActualHours.toFixed(1)}h</div>
              <div className="text-[11px] text-emerald-600 font-medium">Aktueller Leistungsstand</div>
            </div>
            ...
            <div className="bg-white p-4 rounded-2xl border border-slate-200/90 shadow-xs space-y-1">
              ...
              <div className="text-[11px] text-slate-500">Arbeitstage-Hochrechnung</div>
            </div>

            <div className="bg-white p-4 rounded-2xl border border-slate-200/90 shadow-xs space-y-1">
              <div className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">{t.plannedRevenue}</div>
              <div className="text-2xl font-bold text-emerald-700">
                {totalPlannedRevenue.toLocaleString('de-DE', { style: 'currency', currency: 'EUR' })}
              </div>
              <div className="text-[11px] text-slate-500">Erwarteter Monatserlös</div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200/90 shadow-xs overflow-hidden">
            <div className="p-4 bg-slate-50/70 border-b border-slate-200 text-xs font-semibold text-slate-700 flex items-center justify-between">
              <span>Plan-Ist-Vergleich je Projekt & Mitarbeiter</span>
              <span className="text-[11px] text-slate-500 font-normal">
                Hochrechnung = Ist-Stunden × (Arbeitstage Monat / verstrichene Arbeitstage)
              </span>
            </div>
                  ...
                  {comparisonList.length === 0 ? (
                    <tr>
                      <td colSpan={10} className="px-4 py-8 text-center text-slate-400">
                        Keine Forecast-Einträge für {selectedMonth} vorhanden.
                      </td>
                    </tr>
                  ) : (
                      ...
                      <tr key={index} className={`hover:bg-slate-50/70 ${row.isThresholdExceeded ? 'bg-amber-50/30' : ''}`}>
                        ...
                        <td className="px-4 py-3">
                          <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${
                            ...
                            {row.billingModel === 'FIXED_PRICE' ? 'Festpreis' : 'T&M'}
                          </span>
                        </td>
                        ...
                        {currentUser?.role === 'ADMIN' && (
                          <td className="px-4 py-3 text-right font-medium text-slate-800">
                            {row.plannedRevenue.toLocaleString('de-DE', { style: 'currency', currency: 'EUR' })}
                          </td>
                        )}
                        {currentUser?.role === 'ADMIN' && (
                          <td className="px-4 py-3 text-right font-bold text-emerald-700">
                            {(row.plannedRevenue - row.plannedCost).toLocaleString('de-DE', { style: 'currency', currency: 'EUR' })}
                          </td>
                        )}
                        <td className="px-4 py-3 text-center">
                          {row.isThresholdExceeded ? (
                            <span className="inline-flex items-center gap-1 text-[10px] font-bold text-amber-800 bg-amber-100 px-2 py-0.5 rounded-full">
                              <AlertTriangle className="w-3 h-3 text-amber-600" /> &gt;20% Alarm
                            </span>
                          ) : (
          ...
          {/* Modal for adding/adjusting single forecast */}
          {showNewForecastModal && (
            <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-in fade-in">
              <div className="bg-white rounded-2xl max-w-md w-full p-6 space-y-4 shadow-xl border border-slate-200">
                <h3 className="text-base font-bold text-slate-900">
                  Forecast-Monatsplanung erfassen
                </h3>
                <p className="text-xs text-slate-500">
                  Änderungen an einem bestehenden Forecast erzeugen automatisch eine neue versionierte Revision mit Zeitstempel (Section 21).
                </p>

                <form onSubmit={handleSaveForecast} className="space-y-3 text-xs">
                  <div>
                  ...
                  <div>
                    <label className="font-semibold text-slate-700 block mb-1">Plan-Stunden für {selectedMonth}</label>
                    <input
                      id="input-fc-hours"
                      type="number"
                      required
