...
export const CompanyLocationModal: React.FC<CompanyLocationModalProps> = ({ isOpen, onClose }) => {
  const { organization, updateOrganization, currentUser } = useApp();
      ...
      <div className="bg-white rounded-2xl max-w-xl w-full p-6 space-y-5 shadow-2xl border border-slate-200 animate-in zoom-in-95 duration-200">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-bold text-slate-900">
                Unternehmensstandort & Feiertagskalender
              </h3>
              <p className="text-xs text-slate-500 mt-0.5">
                Konfiguration des Hauptsitzes für exakte Arbeitstage- und Forecast-Kalkulation
              </p>
            </div>
          </div>
          <button
        ...
        <div className="bg-slate-50 border border-slate-200 rounded-xl p-3.5 flex items-start gap-2.5 text-xs text-slate-700">
          ...
          <div className="leading-relaxed">
            Da der Unternehmenssitz in <span className="font-bold text-slate-900">{organization?.locationCity || 'Berlin'}</span> liegt,
            werden automatisch alle gesetzlichen Feiertage Berlins (inkl. <span className="font-semibold text-emerald-800">Internationaler Frauentag am 8. März</span>)
            bei der Ermittlung von Soll-Arbeitszeiten und Forecast-Hochrechnungen einberechnet.
          </div>
        </div>
        ...
        <form onSubmit={handleSave} className="space-y-4">
              ...
              >
                {GERMAN_STATES.map(s => (
                  <option key={s.code} value={s.code}>
                    {s.name} ({s.code}) {s.code === 'DE-BE' ? '★ Hauptsitz' : ''}
                  </option>
                ))}
              </select>
            </div>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between text-xs">
              <span className="font-semibold text-slate-700 flex items-center gap-1.5">
                ...
                Gesetzliche Feiertage {currentYear} ({selectedStateObj?.name}):
              </span>
                ...
                <div key={i} className="px-3 py-1.5 flex items-center justify-between hover:bg-slate-50">
                  ...
                  {!h.isNationwide && (
                    <span className="text-[10px] bg-amber-100 text-amber-800 px-1.5 py-0.2 rounded font-semibold">
                      Landesspezifisch ({selectedState})
                    </span>
                  )}
                </div>
              ))}
          ...
          <div className="flex items-center justify-between pt-2 border-t border-slate-100">
            <div className="text-[11px] text-slate-400">
              {currentUser?.role === 'ADMIN' ? 'Admin-Berechtigung aktiv' : 'Änderungen werden mandantenweit angewendet'}
            </div>
