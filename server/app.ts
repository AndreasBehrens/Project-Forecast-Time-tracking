...
import { generateJwtToken, verifyJwtToken } from './authService.js';
import {
...
import { apiNotFoundHandler, errorHandler } from './http/errorHandler.js';
...
export interface CreateAppOptions {
  /**
   * Steuert, ob statische Assets bzw. der Vite-Dev-Server eingebunden werden.
   * In Tests auf `false` gesetzt, um den vollständigen Vite-Start zu vermeiden.
   */
  serveStatic?: boolean;
}

/**
 * Erstellt die Express-App inkl. Middleware, API-Routen und zentraler
 * Fehlerbehandlung – aber OHNE `listen()`, Datenbank-Initialisierung oder
 * Graceful-Shutdown. Dadurch ist die App isoliert (z. B. via supertest) testbar,
 * während {@link file://../server.ts} den Produktiv-Bootstrap übernimmt.
 */
export async function createApp(options: CreateAppOptions = {}): Promise<express.Express> {
  ...
  app.post('/api/time-entries', validateBody(CreateTimeEntrySchema), (req, res, next) => {
    try {
      const actorId = getActorId(req);
      const entry = storage.createTimeEntry(req.body, actorId);
      res.status(201).json(entry);
    } catch (err: any) {
      next(new BadRequestError(err.message || 'Fehler beim Erstellen der Zeiterfassung'));
    }
  });

  app.put('/api/time-entries/:id', validateBody(UpdateTimeEntrySchema), (req, res, next) => {
    try {
      const actorId = getActorId(req);
      ...
      next(err instanceof NotFoundError ? err : new BadRequestError(err.message || 'Fehler beim Aktualisieren der Zeiterfassung'));
    }
  });

  app.delete('/api/time-entries/:id', (req, res, next) => {
    try {
      const actorId = getActorId(req);
      const ok = storage.deleteTimeEntry(req.params.id, actorId);
      if (!ok) throw new NotFoundError('Zeiteintrag nicht gefunden');
      res.json({ success: true });
    } catch (err: any) {
      next(err instanceof NotFoundError ? err : new BadRequestError(err.message || 'Fehler beim Löschen des Zeiteintrags'));
    }
  });
  ...
  app.post('/api/working-time', validateBody(CreateWorkingTimeSchema), (req, res, next) => {
    try {
      const actorId = getActorId(req);
      const entry = storage.createOrUpdateWorkingTime(req.body, actorId);
      res.json(entry);
    } catch (err: any) {
      next(new BadRequestError(err.message || 'Fehler beim Speichern der Arbeitszeit'));
    }
  });
  ...
  app.post('/api/forecasts', validateBody(CreateForecastSchema), (req, res, next) => {
    try {
      const actorId = getActorId(req);
      const fc = storage.saveForecast(req.body, actorId);
      res.status(201).json(fc);
    } catch (err: any) {
      next(new BadRequestError(err.message || 'Fehler beim Speichern der Planung'));
    }
  });
  ...
  // -------------------------------------------------------------
  // API 404 – unbekannte /api-Routen liefern strukturiertes JSON
  // (muss VOR dem SPA-Catch-All stehen).
  // -------------------------------------------------------------
  app.use('/api', apiNotFoundHandler);

  // -------------------------------------------------------------
  // VITE / STATIC SERVING (optional, für Tests deaktivierbar)
  // -------------------------------------------------------------
  if (serveStatic) {
  ...
  // -------------------------------------------------------------
  // ZENTRALE FEHLER-MIDDLEWARE (muss die LETZTE Middleware sein)
  // -------------------------------------------------------------
